...
```{r}
GDPCA <- read_csv("GDPCA.csv") %>% 
  mutate(DATE = lubridate::parse_date_time(DATE,orders='dmy'),
    year = year(DATE))

head(GDPCA)

hibbs <- inner_join(GDPCA,hibbs,by='year') %>%
  mutate(gdp = as.numeric(substr(gdp,1,4)))
...
```{r}
ggplot(hibbs,aes(x=gdp,y=vote)) + 
  geom_line(linetype='dashed') +
  geom_smooth(method='lm',se=F) +
  geom_text(aes(x=gdp,y=vote,label=year)) +
  geom_text(aes(x=5.4,y=46.5,label='Ross Perot?')) +
  geom_text(aes(x=5.4,y=49,label='Gerald Ford')) +
  labs(x="Real GDP growth (%)",y="Incumbent party's vote share")
```
...
m1.lm <- lm(data=hibbs,vote~gdp+growth)
m1.step <- step(m1.lm,k=log(n)) # BIC model selection
```
